...
// get address of bios parameter block
static struct BPB *get_fs_bpb(void)
{
    uint32_t lba = *(uint32_t *)((uint64_t)FS_BASE + 0x1be + 8);

    return (struct BPB *)((uint64_t)FS_BASE + lba * 512); // assume sector is 512 bytes large
}
...
static uint32_t search_file(char *path)
{
    ...
    if (status == true)
    {
        ...
        for (uint32_t i = 0; i < root_entry_count; i++)
        {
            ...
            //we dont support long file name
