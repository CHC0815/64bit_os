...
void init_idt(void)
{
    ...
    init_idt_entry(&vectors[0x80], (uint64_t)sysint, 0xee); // system call print
...
void handler(struct TrapFrame *tf)
{
    ...
    case 39:
        isr_value = read_isr();
        if ((isr_value & (1 << 7)) != 0) // check if it is a real interrupt
        {
            eoi();
        }
        break;
    ...
    default:
        // check if exception is generated in usermode --> lower 3 bits are set
        if ((tf->cs & 3) == 3)
        {
            printk("Exception is %d\n", tf->trapno);
            exit(); //terminate the current process (with the error)
        }
        else
        {
